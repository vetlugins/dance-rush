	...
	static public function selector($current = TRUE)
	{
		...
		{				
			...
			if($lang === Request::$lang)
			{
				...
				if($current)
				{
					$selectors[$lang] = '<span class="multilang-selected multilang-'.$lang.'" data-toggle="tooltip" data-placement="bottom" title="'.$languages[$lang]['label'].'">'.$languages[$lang]['label'].'</span>';
				}				
			}
			else
			{	
				...
				if($route !== NULL)
				{
					$selectors[$lang] = HTML::anchor($route->uri($params), $languages[$lang]['label'], array('data-toggle'=>'tooltip','data-placement'=>'bottom','class' => 'multilang-selectable multilang-'.$lang, 'title' => $languages[$lang]['label']));
				}
			}
		}
