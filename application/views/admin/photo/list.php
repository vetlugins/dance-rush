...
if(!empty($album->cover())) $icon_image = '/uploads/photo/medium/'.$album->cover();
else $icon_image = '/uploads/system/nophoto.jpg';

if(count($album->photos()) > 0) $delete = '<a title="Есть фото" data-toggle="tooltip" class="btn btn-sm btn-default"><i class="fa fa-trash-o"></i></a>';
...
echo '<li id="array_order_'.$album->id.'">
           <div class="item">
                <div class="pull-left" style="padding: 15px 0 0 10px">
                    <a href="/admin/'.$params['module'].'/album/'.$album->url.'" data-toggle="tooltip" title="Перейти к фотографиям"><strong>'.$album->title.'</strong></a>
                </div>
                <div class="pull-right padding-sm">
                    <a href="/admin/'.$params['module'].'/album/'.$album->id.'" title="Перейти к фотографиям" data-toggle="tooltip" class="btn btn-default btn-sm" ><i class="fa fa-photo"></i> '.count($album->photos()).'</a>
